...
export class EventosSolicitudComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.lForm.get('bodorigendesc').setValue(this._Solicitud.boddestinodesc);
    this.lForm.get('boddestinodesc').setValue(this._Solicitud.bodorigendesc);
  ...
  BuscaEventosSolictudes () {
    this._solicitudService.BuscaEventosSolicitud(this.hdgcodigo, this.esacodigo, this.cmecodigo, this._Solicitud.soliid, this.servidor).subscribe(
      response => {
        if (response != null) {
