...
// ModificarOrdenCompra is...
func ModificarOrdenCompra(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo crear de transaccion para modificar orden de compra",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo ejecucion package modificar orden de compra",
			Error:   err,
		})
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo rollback package modificar orden de compra",
				Error:   err,
			})
		}
	}
	...
	if SRV_MESSAGE != "1000000" {
		...
		logger.Trace(logs.InformacionLog{Mensaje: "Rollback package modificar orden de compra " + SRV_MESSAGE})
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo commit package modificar orden de compra",
				Error:   err,
			})
			defer transaccion.Rollback()
			...
			logger.Trace(logs.InformacionLog{Mensaje: "Exito commit package modificar orden de compra"})
