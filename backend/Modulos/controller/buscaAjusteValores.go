...
// BuscaAjusteValores is...
func BuscaAjusteValores(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamBuscaAjustesValores
	...
	res := models.ParamBuscaAjustesValores{}
	...
	logger.Trace(logs.InformacionLog{
		Query:   query,
		Mensaje: "Query busca ajuste valores",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   query,
			Mensaje: "Se cayo query busca ajuste valores",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan busca ajuste valores",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
