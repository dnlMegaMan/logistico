...
// GetURLConsultaLibControlados is...
func GetURLConsultaLibControlados(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamConsultaLibControlados
	...
	res := models.ParamConsultaLibControlados{}
	...
	qry = " BEGIN"
	qry = qry + " PKG_RPT_CONSULTALIBCONTROLADOS.PRO_RPT_CONSULTALIBCONTROLADOS("
	...
	qry = qry + " ," + strconv.Itoa(res.PiLibCID)
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte consulta consulta libros controlados",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte consulta consulta libros controlados",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	sReporte = "consultalibcontrolados.rpt"
