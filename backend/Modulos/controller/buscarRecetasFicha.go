...
// BuscarRecetasFicha is...
func BuscarRecetasFicha(w http.ResponseWriter, r *http.Request) {
	...
	query = query + " AND RECE_SUBRECETA <> 0 " // Deja afuera las recetas Madre de las recetas programadas
	...
	if res.RECETIPO != "" {
		query = query + " AND clin_far_recetas.RECE_TIPO = '" + res.RECETIPO + "' "
	}
	if res.RECENUMERO != 0 {
	...
	if res.FECHAINICIO != "" && res.FECHAHASTA != "" {
		query = query + " and clin_far_recetas.RECE_FECHA between TO_DATE('" + res.FECHAINICIO + " 00:00:00','YYYY-MM-DD HH24:MI:SS') and TO_DATE ('" + res.FECHAHASTA + " 23:59:59' ,'YYYY-MM-DD HH24:MI:SS') "
	}
	...
	if res.CLINOMBRES != "" || res.CLIAPEPATERNO != "" || res.CLIAPEMATERNO != "" {
		...
		if res.CLIAPEPATERNO != "" {
			nombreDer = nombreDer + "%" + strings.ToUpper(res.CLIAPEPATERNO) + "%"
		}
		if res.CLIAPEMATERNO != "" {
			nombreDer = nombreDer + "%" + strings.ToUpper(res.CLIAPEMATERNO) + "%"
		}
		if res.CLINOMBRES != "" {
			nombreDer = nombreDer + "%" + strings.ToUpper(res.CLINOMBRES) + "%"
		}
		nombreIzq := ""
		if res.CLINOMBRES != "" {
			nombreIzq = nombreIzq + "%" + strings.ToUpper(res.CLINOMBRES) + "%"
		}
		if res.CLIAPEPATERNO != "" {
			nombreIzq = nombreIzq + "%" + strings.ToUpper(res.CLIAPEPATERNO) + "%"
		}
		if res.CLIAPEMATERNO != "" {
			nombreIzq = nombreIzq + "%" + strings.ToUpper(res.CLIAPEMATERNO) + "%"
		}
		query = query + " AND (RECE_NOMBRE_PACIENTE LIKE ('" + nombreDer + "') OR RECE_NOMBRE_PACIENTE LIKE ('" + nombreIzq + "'))"
	}
	...
	logger.Trace(logs.InformacionLog{
		Query:   query,
		Mensaje: "Query buscar recetas ficha",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   query,
			Mensaje: "Se cayo query buscar recetas ficha",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan buscar recetas ficha",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
