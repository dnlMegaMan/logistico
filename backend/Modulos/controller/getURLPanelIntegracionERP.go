...
// GetURLPanelIntegracionERP is...
func GetURLPanelIntegracionERP(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamPanelIntegracionERP
	...
	res := models.ParamPanelIntegracionERP{}
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_PANELINTEGRACIONERPDET .PRO_RPT_PANELINTEGRACIONERPDET("
	...
	qry = qry + " ," + strconv.Itoa(res.PiSoliCon)
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte panel integracion ERP",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte panel integracion ERP",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
