...
// GetURLConsultaRecetaProg is...
func GetURLConsultaRecetaProg(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamConsultaRecetaProg
	...
	res := models.ParamConsultaRecetaProg{}
	...
	qry = " BEGIN"
	qry = qry + " PKG_RPT_RECETASPROGRAMADAS.PRO_RPT_RECETASPROGRAMADAS("
	...
	qry = qry + " ," + strconv.Itoa(res.PCliID)
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte consulta receta programada",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte consulta receta programada",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	sReporte = "recetasprogramadas.rpt"
