...
// ObtieneURLinfCoberturaGeneral is...
func ObtieneURLinfCoberturaGeneral(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamInfCoberturaGeneral
	...
	res := models.ParamInfCoberturaGeneral{}
	json.Unmarshal([]byte(output), &res)

	logger.Info(logs.InformacionLog{JSONEntrada: res, Mensaje: "JSON de entrada"})

	var PTipoReport string
	var PMesesConsumo int
	var PDias int
	var PTiporeg string
	var PHdgCodigo int
	var PEsaCodigo int
	...
	PMesesConsumo = res.PiMesesConsumo
	PDias = res.PiDias
	...
	_, err = db.Exec("Begin PKG_RPT_COBERTURA_GENERAL.PRO_RPT_COBERTURA_GENERAL(:In_MesesConsumo,:In_Dias,:In_Tiporeg,:In_IdReport,:In_HdgCodigo,:In_EsaCodigo,:In_CmeCodigo); end;", PMesesConsumo, PDias, PTiporeg, intIDReport, PHdgCodigo, PEsaCodigo, PCmeCodigo)

	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo query al ejecutar pkg PKG_RPT_COBERTURA_GENERAL.PRO_RPT_COBERTURA_GENERAL",
			Error:   err,
			Contexto: map[string]interface{}{
				"PMesesConsumo": PMesesConsumo, "PDias": PDias, "PTiporeg": PTiporeg,
	...
	sReporte = "logistico/rpt_cobertura_general.rpt"
	sTipo = PTipoReport
	...
	logger.Trace(logs.InformacionLog{
		Mensaje: "Datos informe cobertura general",
		Contexto: map[string]interface{}{
