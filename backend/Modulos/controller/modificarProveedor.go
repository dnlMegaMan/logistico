...
func ModificarProveedor(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo creacion de transaccion para modificar proveedor",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	logger.Trace(logs.InformacionLog{
		Query:    QUERY,
		Mensaje:  "Query modificar proveedor",
	...
	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo package modificar proveedor",
			Error:   err,
		})
	...
	if SRV_MESSAGE != "1000000" {
		...
		logger.Trace(logs.InformacionLog{Mensaje: "Rollback modificar proveedor " + SRV_MESSAGE})
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo commit modificar proveedor",
				Error:   err,
			})
			defer transaccion.Rollback()
			...
			logger.Trace(logs.InformacionLog{Mensaje: "Exito commit modificar proveedor"})
