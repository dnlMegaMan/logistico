...
// SelBodInvMedControlados is...
func SelBodInvMedControlados(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.PBodInvMedControlados
	...
	res := models.PBodInvMedControlados{}
	...
	qry += " WHERE  fboi_fbod_codigo = " + strconv.Itoa(res.CodBodegaControlados)
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query seleccionar bodega inv medicamentos controlados",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query seleccionar bodega inv medicamentos controlados",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	retornoValores := []models.BodInvMedControlados{}
	for rows.Next() {
		valores := models.BodInvMedControlados{}
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan seleccionar bodega inv medicamentos controlados",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
