...
// GetURLAdministracionSolicPac is...
func GetURLAdministracionSolicPac(w http.ResponseWriter, r *http.Request) {
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_ADMINSOLICPAC.PRO_RPT_ADMINSOLICPAC("
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte administracion solicitudes paciente",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte administracion solicitudes paciente",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
