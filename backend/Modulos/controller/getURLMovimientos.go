...
// GetURLMovimientos is...
func GetURLMovimientos(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamInformeMovimientos
	...
	res := models.ParamInformeMovimientos{}
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_MOVIMIENTOS.PRO_RPT_MOVIMIENTOS("
	...
	qry = qry + " ," + strconv.Itoa(res.PiMovfID)
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte movimientos",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte movimientos",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
