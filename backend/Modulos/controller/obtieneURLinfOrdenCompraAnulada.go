...
// ObtieneURLinfOrdenCompraAnulada is...
func ObtieneURLinfOrdenCompraAnulada(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamInfOrdenCompraAnulada
	...
	res := models.ParamInfOrdenCompraAnulada{}
	...
	_, err = db.Exec("Begin PKG_RPT_CLIN_OC_MIN_ANULADAS.PRO_RPT_CLIN_OC_MIN_ANULADAS(:In_IdReport,:In_HdgCodigo,:In_EsaCodigo,:In_CmeCodigo); end;", intIDReport, PHdgCodigo, PEsaCodigo, PCmeCodigo)

	if err != nil {
		logger.Error(logs.InformacionLog{
			Mensaje: "Se cayo query al ejecutar pkg PKG_RPT_CLIN_OC_MIN_ANULADAS.PRO_RPT_CLIN_OC_MIN_ANULADAS",
			Error:   err,
	...
	sReporte = "logistico/rpt_oc_anuladas.rpt"
	sTipo = PTipoReport
	...
	logger.Trace(logs.InformacionLog{
		Mensaje: "Datos informe orden de compra anulada",
		Contexto: map[string]interface{}{
