...
// GetURLAdministracionSolicBod is...
func GetURLAdministracionSolicBod(w http.ResponseWriter, r *http.Request) {
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_ADMINSOLICBOD.PRO_RPT_ADMINSOLICBOD("
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte administracion solicitudes bodega",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte administracion solicitudes bodega",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
