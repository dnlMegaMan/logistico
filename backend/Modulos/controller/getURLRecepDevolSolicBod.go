...
// GetURLRecepDevolSolicBod is...
func GetURLRecepDevolSolicBod(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamInformesSolicBod
	...
	res := models.ParamInformesSolicBod{}
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_RECEPDEVOLSOLICBOD.PRO_RPT_RECEPDEVOLSOLICBOD("
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte recepcion devolucion solicitud bodega",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte recepcion devolucion solicitud bodega",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	sReporte = "recepdevolsolicbod.rpt"
