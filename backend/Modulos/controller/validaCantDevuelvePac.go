...
// ValidaCantDevuelvePac is...
func ValidaCantDevuelvePac(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.PValidaCantDevPac
	...
	qry += " AND movf_cliid = " + strconv.Itoa(res.CliID)
	qry += " AND movf_tipo = 140"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query valida cant devuelve pac",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query valida cant devuelve pac",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan valida cant devuelve pac",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
	...
	valores := models.GetValidaCantDevPac{
		ValidaCant: validacant,
		Cantidad:   cantidad,
	}

	retornoValores := []models.GetValidaCantDevPac{valores}
