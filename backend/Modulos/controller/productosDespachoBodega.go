...
// ProductosDespachoBodega is...
func ProductosDespachoBodega(w http.ResponseWriter, r *http.Request) {
	...
	qry = "SELECT soli_id, sode_id,  mfde_lote, mfde_lote_fechavto, cantidad_movdet_30 cantidad_recepcionada, SUM(cantidad_movdet_100) cantidad_despachada"
	qry = qry + " ,sode_mein_id, sode_mein_codmei, des, sode_cant_soli, sode_cant_recepcionado, sode_cant_devo"
	qry = qry + " ,(SUM(cantidad_movdet_100) - cantidad_movdet_30) cantidad_pendiente_recepcion"
	qry = qry + " FROM"
	qry = qry + " ("
	...
	qry = qry + " , mdet.mfde_cantidad  cantidad_movdet_100 "
	...
	qry = qry + "           and MD.MFDE_TIPO_MOV = 30 "
	qry = qry + "                                ),0)   ) cantidad_movdet_30"
	...
	qry = qry + " AND mdet.MFDE_TIPO_MOV in (100,102) "
	qry = qry + " )"
	qry = qry + " group by soli_id, sode_id, mfde_lote, mfde_lote_fechavto, cantidad_movdet_30"
	qry = qry + " , sode_mein_id, sode_mein_codmei, des, sode_cant_soli, sode_cant_recepcionado, sode_cant_devo"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query productos despacho bodega",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query productos despacho bodega",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	retornoValores := []models.ProdDespachoBod{}
	for rows.Next() {
		valores := models.ProdDespachoBod{}
		...
		err := rows.Scan(
			...
			&valores.CantPendienteRecepcion,
		)

		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan productos despacho bodega",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
