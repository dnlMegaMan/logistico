...
// GetURLDespachoReceta is...
func GetURLDespachoReceta(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamDespachoReceta
	...
	res := models.ParamDespachoReceta{}
	...
	qry = " BEGIN"
	qry = qry + " PKG_RPT_DESPACHORECETA.PRO_RPT_DESPACHORECETA("
	...
	qry = qry + " ," + strconv.Itoa(res.PiTipo)
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte despacho receta",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte despacho receta",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
