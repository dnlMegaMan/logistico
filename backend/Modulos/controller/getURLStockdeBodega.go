...
// GetURLStockdeBodega is...
func GetURLStockdeBodega(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamInfStockDeBodega
	...
	res := models.ParamInfStockDeBodega{}
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_STOCKDEBODEGA.PRO_RPT_STOCKDEBODEGA("
	...
	qry = qry + " ," + strconv.Itoa(res.PiFbodCodigo)
	qry = qry + " , '" + res.PiFboCodigoBodega + "'"
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte reporte stock de bodega",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte reporte stock de bodega",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
