...
// SelSuministroaOrigen is...
func SelSuministroaOrigen(w http.ResponseWriter, r *http.Request) {
	...
	strQuery += " where  rel.fbod_codigo_solicita = bod.fbod_codigo"
	strQuery += " and bod.fbod_estado = 'S'"
	strQuery += " and rel.hdgcodigo = bod.hdgcodigo"
	//strQuery += " and rel.esacodigo = bod.esacodigo"
	...
	strQuery += " and rel.fbod_codigo_entrega =  " + strconv.Itoa(PiBodCodigoSolicita) + " "
	...
	logger.Trace(logs.InformacionLog{
		Query:   strQuery,
		Mensaje: "Query seleccionar suministro origen",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   strQuery,
			Mensaje: "Se cayo query seleccionar suministro origen",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan seleccionar suministro origen",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
