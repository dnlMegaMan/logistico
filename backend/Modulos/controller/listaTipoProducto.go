...
// Listatipoproducto is...
func Listatipoproducto(w http.ResponseWriter, r *http.Request) {
	...
	query += " to_char(FPAR_FECHA_CREACION,'dd-mm-yyyy') FPAR_FECHA_CREACION, FPAR_MODIFICABLE, FPAR_INCLUYE_CODIGO, FPAR_VALOR from clin_far_param where fpar_tipo = 27 and fpar_codigo >0 "
	...
	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   query,
			Mensaje: "Se cayo query lista tipo producto",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan lista tipo producto",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
