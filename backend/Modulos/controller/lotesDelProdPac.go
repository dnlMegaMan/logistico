...
// LotesDelProdPac is...
func LotesDelProdPac(w http.ResponseWriter, r *http.Request) {
	...
	qry = qry + " AND movf_tipo = 140"
	qry = qry + " AND movf_cliid = " + strconv.Itoa(res.CliID)
	qry = qry + " AND movf_id  =  mfde_movf_id"
	qry = qry + " AND mfde_mein_codmei = trim('" + res.CodMei + "')"
	qry = qry + " AND mfde_tipo_mov = 140"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query lotes del prod pac",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query lotes del prod pac",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan lotes del prod pac",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
