...
// ProductosRecepcionBodega is...
func ProductosRecepcionBodega(w http.ResponseWriter, r *http.Request) {
	...
	qry = qry + " , to_char(mdet.MFDE_FECHA,'yyyy-mm-dd hh24:mi:ss') fecha_movdet "
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query productos recepcion bodega",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query productos recepcion bodega",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan productos recepcion bodega",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
