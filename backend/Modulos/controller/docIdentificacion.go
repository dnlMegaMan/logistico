...
// DocIdentificacion is...
func DocIdentificacion(w http.ResponseWriter, r *http.Request) {
	...
	logger.Trace(logs.InformacionLog{
		Query:   query,
		Mensaje: "Query doc identificacion",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   query,
			Mensaje: "Se cayo query doc identificacion",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	retornoValores := []models.Identificacion{}
	for rows.Next() {
		valores := models.Identificacion{}

		err := rows.Scan(
			&valores.DocIdentCodigo,
			&valores.DocIdentDescripcion,
		)

		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan doc identificacion",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
