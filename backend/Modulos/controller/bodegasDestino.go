...
// BodegasDestino is...
func BodegasDestino(w http.ResponseWriter, r *http.Request) {
	...
	retornoValores := []models.Bodegasdest{}
	for rows.Next() {
		valores := models.Bodegasdest{}
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan bodegas destino",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
