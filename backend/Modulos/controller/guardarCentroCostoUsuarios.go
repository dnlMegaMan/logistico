...
// GuardarCentroCostoUsuarios is...
func GuardarCentroCostoUsuarios(w http.ResponseWriter, r *http.Request) {
	...
	//if err != nil {
	//	log.Println("ERROR (guardarCentroCostoUsuarios): conectarBaseDeDatos, res.SERVIDOR: ", res.SERVIDOR)
	...
	for i := range detalletalle {
		...
		if detalletalle[i].ACCION == "I" {
			insertando = 1
			qryIns1 = qryIns1 + "into  CLIN_FAR_CENTROCOSTO_USUARIOS (ID_USUARIO,ID_CENTROCOSTO,HDGCODIGO,ESACODIGO,CMECODIGO) values ( "
			qryIns1 = qryIns1 + strconv.Itoa(detalletalle[i].IDUSUARIO)
			qryIns1 = qryIns1 + " ," + strconv.Itoa(detalletalle[i].IDCENTROCOSTO)
		...
		if detalletalle[i].ACCION == "E" {
			qryUpd1 = qryUpd1 + " delete CLIN_FAR_CENTROCOSTO_USUARIOS "
			qryUpd1 = qryUpd1 + " Where ID_USUARIO =" + strconv.Itoa(detalletalle[i].IDUSUARIO)
			qryUpd1 = qryUpd1 + " and ID_CENTROCOSTO =" + strconv.Itoa(detalletalle[i].IDCENTROCOSTO)
	...
	if transaccion == 1 {
		...
		logger.Trace(logs.InformacionLog{
			Query:   query,
			Mensaje: "Query guardar centro costo usuarios",
		})

		if err != nil {
			logger.Error(logs.InformacionLog{
				Query:   query,
				Mensaje: "Se cayo query guardar centro costo usuarios",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
