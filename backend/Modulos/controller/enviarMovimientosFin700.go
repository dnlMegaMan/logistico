...
// EnviarmovimientosFin700 is...
func EnviarmovimientosFin700(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamFin700Movimiento
	...
	res := models.ParamFin700Movimiento{}
	json.Unmarshal([]byte(output), &res)
	models.EnableCors(&w)

	xmlLogger.SetUsuario(res.Usuario)
	xmlLogger.Info(logs.InformacionLog{JSONEntrada: res, Mensaje: "JSON de entrada"})
	...
	if res.SobreGiro {
		query = query + "     and det.int_erp_error not like ('%sobregiro%') "
		query = query + "     and det.int_erp_error not like ('%saldo%') "
	}
	query = query + " union "
	query = query + "SELECT  "
	query = query + "     nvl(cab.esacodigo, 0) AS Empresa  "
	...
	if res.SobreGiro {
		query = query + "     and dev.int_erp_error not like ('%sobregiro%') "
		query = query + "     and dev.int_erp_error not like ('%saldo%') "
	}
	...
	if rows != nil {
		for rows.Next() {
			...
			case 16: // ingreso de fraccionamiento
				...
				queryAux = queryAux + " where reg.REGLA_BODEGACODIGO = (SELECT BOD.FBOD_CODIGO FROM CLIN_FAR_BODEGAS BOD WHERE "
				queryAux = queryAux + " BOD.ESACODIGO = esacodigo AND BOD.FBO_CODIGOBODEGA = " + BodegaOrigen + "))"
		...
		if transaccion == 1 {
			...
			if res.SoliID != 0 {
				xmlLogger.Info(logs.InformacionLog{
					Mensaje: "Body XML : Solicitud " + strconv.Itoa(res.SoliID) + " " + string(j),
				...
				xmlLogger.Info(logs.InformacionLog{
					Mensaje: "Body XML : Agrupador " + strconv.Itoa(res.IDAgrupador) + " " + string(j),
					Contexto: map[string]interface{}{
						"xml":       string(j),
			...
			if len(resultado.DetalleCodigoError.DetalleCodigo) > 0 {
				...
				for _, element := range resultado.DetalleCodigoError.DetalleCodigo {
					...
					case "NORMAL":
						...
						queryUpdate = queryUpdate + ",int_erp_fecha = sysdate "
				...
				case "NORMAL":
					...
					if OperacionConsumoRef != "0" {
						...
						if errUpdateOr != nil {
							...
							http.Error(w, errUpdateOr.Error(), http.StatusInternalServerError)
							return
						}
			...
			if reproceso {
				if inNumeromovimiento > 10 {
					...
					xmlLogger.Info(logs.InformacionLog{Mensaje: "FOLIO : " + strconv.Itoa(FOLIO)})
				}
			}
		}
	}
