...
// GetURLDevolverSolicBod is...
func GetURLDevolverSolicBod(w http.ResponseWriter, r *http.Request) {
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_DEVOLVERSOLICBOD.PRO_RPT_DEVOLVERSOLICBOD("
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte devolver solicitud",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte devolver solicitud",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
