...
// GetURLControlStockMin is...
func GetURLControlStockMin(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamControlStockMin
	...
	res := models.ParamControlStockMin{}
	...
	qry = " BEGIN"
	qry = qry + " PKG_RPT_CONTROLSTOCKMIN.PRO_RPT_CONTROLSTOCKMIN("
	...
	qry = qry + " , '" + res.FechaInicio + "'"
	qry = qry + " , '" + res.FechaTermino + "'"
	qry = qry + " ," + strconv.Itoa(res.IDBodegaSolicita)
	qry = qry + " ," + strconv.Itoa(res.IDBodegaSuministro)
	qry = qry + " ," + strconv.Itoa(res.IDArticulo)
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte control stock minimo",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte control stock minimo",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
