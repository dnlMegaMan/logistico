...
// GetURLMonitorERP is...
func GetURLMonitorERP(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var msg models.ParamMonitorERP
	...
	var qry string
	qry = " BEGIN"
	qry = qry + " PKG_RPT_MONITORERP.PRO_RPT_MONITORERP("
	...
	qry = qry + " ," + strconv.Itoa(res.PiPlanID)
	qry = qry + " ," + strconv.Itoa(res.PiPlanTipo)
	qry = qry + " );"
	qry = qry + " END;"
	...
	logger.Trace(logs.InformacionLog{
		Query:   qry,
		Mensaje: "Query package reporte monitor ERP",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   qry,
			Mensaje: "Se cayo query package reporte monitor ERP",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
