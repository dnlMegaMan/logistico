...
// SelBodegaFraccionable is...
func SelBodegaFraccionable(w http.ResponseWriter, r *http.Request) {
	...
	strQuery += " ,nvl(fbod_tipo_bodega, ' ') fbod_tipo_bodega, nvl(fbod_tipoproducto, ' ') fbod_tipoproducto"
	...
	strQuery += " and FBO_FRACCIONABLE = 'S' "
	...
	logger.Trace(logs.InformacionLog{
		Query:   strQuery,
		Mensaje: "Query seleccionar bodega fraccionable",
	})

	if err != nil {
		logger.Error(logs.InformacionLog{
			Query:   strQuery,
			Mensaje: "Se cayo query seleccionar bodega fraccionable",
			Error:   err,
		})
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	for rows.Next() {
		...
		if err != nil {
			logger.Error(logs.InformacionLog{
				Mensaje: "Se cayo scan seleccionar bodega fraccionable",
				Error:   err,
			})
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
