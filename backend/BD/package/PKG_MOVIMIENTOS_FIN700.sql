create or replace PACKAGE  "PKG_MOVIMIENTOS_FIN700" AS

PROCEDURE PRO_MOVIMIENTOS_FIN700
/*  Registra Solicitudes  */
    ( SRV_Message       In Out     Varchar2                        /*  Parametro de uso interno  */ 
     ,In_Json           In         CLOB
     ,Out_Json          Out        CLOB     
    );
END PKG_MOVIMIENTOS_FIN700;
/
create or replace PACKAGE BODY "PKG_MOVIMIENTOS_FIN700" AS
...
/*
** Nombre			: PKG_MOVIMIENTOS_FIN700.PRO_MOVIMIENTOS_FIN700	
** Sistema			: Logistico
            ...
            BEGIN
                ...
                /*BEGIN
                    ntracelog_pkg.graba_log(
                        'PKG_MOVIMIENTOS_FIN700', -- varchar(1000)
                        ' LINES 184',
                        ' srv_message              : ' || srv_message ||               
                        '| In_Json                 : ' || In_Json     ||
                        '| V_SOLI_CODSERVICIOACTUAL: |' || V_SOLI_CODSERVICIOACTUAL || '|', -- varchar(500)
                       null -- Clob
                    );
                END;*/
                    ...
                    BEGIN
						UPDATE CLIN_FAR_SOLICITUDES
						 ...
						 INSERT INTO CLIN_FAR_EVENTOSOLICITUD ( 
							SOLI_ID ,CODEVENTO,FECHA,OBSERVACION,USUARIO
						 ) VALUES (
							  V_SOLI_ID
							, 80
							, SYSDATE
								...
								FROM  JSON_TABLE( (SELECT DOC FROM JSON) , '$.solicitudesdet[*]' 
                                 ...
                                 BEGIN
									...
									INSERT INTO CLIN_FAR_DETEVENTOSOLICITUD ( 
										SODE_ID,SOLI_ID,CODEVENTO,FECHA,OBSERVACION,CANTIDAD,USUARIO,LOTE,FECHAVTO
									) VALUES (
									  D.SODEID
									, D.SOLIID
									, 80
