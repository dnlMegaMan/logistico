create or replace PACKAGE PKG_PROCESA_MOVIMIENTOS_ART_CONTROLADO As

Procedure PROCESA_MOVIMIENTOS_ART_CONTROLADO
...
End PKG_PROCESA_MOVIMIENTOS_ART_CONTROLADO;
/
create or replace PACKAGE BODY  PKG_PROCESA_MOVIMIENTOS_ART_CONTROLADO As
...
---
            ...
            v_referencia                        clin_far_movimdet.mfde_referencia_contable%type;
            v_lote                              clin_far_movimdet.mfde_lote%type;
            v_KADE_ERROR_ERP                    TMP_CIERRE_KARDEX_DET.KADE_ERROR_ERP%TYPE;

            V_KADE2_ESACODIGO                   TMP_CIERRE_KARDEX_MATCH.KADE2_ESACODIGO%TYPE;
            V_KADE2_MEIN_ID                     TMP_CIERRE_KARDEX_MATCH.KADE2_MEIN_ID%TYPE;
            V_KADE2_MEIN_CODIGO                 TMP_CIERRE_KARDEX_MATCH.KADE2_MEIN_CODIGO%TYPE;
            V_KADE2_FBOD_CODIGO                 TMP_CIERRE_KARDEX_MATCH.KADE2_FBOD_CODIGO%TYPE;
            V_KADE2_STOCK_KARDEX                TMP_CIERRE_KARDEX_MATCH.KADE2_STOCK_KARDEX%TYPE;
            V_KADE2_STOCK_BODEGA                TMP_CIERRE_KARDEX_MATCH.KADE2_STOCK_BODEGA%TYPE;
        ...
        BEGIN
            ...
            insert into PASO_ERRORES_EO(DESCRIPCION, FECHA) values ('v_CKAR_ID_CierreAnterior: |' || v_CKAR_ID_CierreAnterior || '|' || In_HdgCodigo || '|' ||  In_CMECODIGO || '|' ||  In_CodBodega, sysdate ); commit;
            ...
            Exception When Others Then
                v_SQLERRM := SQLERRM;
                insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('ERROR : ' || v_SQLERRM || '' , sysdate ); commit;
            --Goto PKG_Exit;
            end;
            insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('v_CKAR_FECHA_APERTURA : ' || v_CKAR_FECHA_APERTURA || '' , sysdate ); commit;

            v_CKAR_FECHA_CIERRE:= TO_DATE(In_Fecha, 'dd/mm/yyyy HH24:MI:SS');
            v_CKAR_PERIODO:= to_number(to_char(TO_DATE(In_Fecha, 'dd/mm/yyyy HH24:MI:SS'), 'yyyymm'));
            ...
            for cur in (SELECT FBOI_MEIN_ID, FBOI_STOCK_ACTUAL
                ...
                loop
                    ...
                    insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('LINEA 131 v_CKAR_ID_CierreAnterior : ' || v_CKAR_ID_CierreAnterior || ' cur.FBOI_MEIN_ID : ' || cur.FBOI_MEIN_ID , sysdate ); commit;
                    begin
                    ...
                    insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('LINEA 144 v_CKAR_FECHA_APERTURA : ' || v_CKAR_FECHA_APERTURA || ' cur.FBOI_MEIN_ID : ' || cur.FBOI_MEIN_ID, sysdate ); commit;
                        ...
                        FROM
                            (
                                SELECT
                                    ...
                                    0 MDEV_ID,
                                    MDE.MFDE_MEIN_ID,
                                    MDE.MFDE_FECHA      MFDE_FECHA,
                                    MDE.INT_ERP_ERROR   INT_ERP_ERROR
                                FROM
                                    CLIN_FAR_MOVIMDET MDE
                                WHERE
                                    ((mde.MFDE_FECHA) between (TO_DATE(TO_CHAR(v_CKAR_FECHA_APERTURA, 'DD/MON/YY'))) and   v_CKAR_FECHA_CIERRE or v_CKAR_FECHA_APERTURA is null)
                                  AND MFDE_MEIN_ID = cur.FBOI_MEIN_ID
                                UNION ALL
                                SELECT
                                    DEV.MDEV_MFDE_ID    MFDE_ID,
                                    DEV.MDEV_ID,
                                    MFDE_MEIN_ID,
                                    DEV.MDEV_FECHA      MFDE_FECHA,
                                    DEV.INT_ERP_ERROR   INT_ERP_ERROR
                                FROM
                                    CLIN_FAR_MOVIM_DEVOL   DEV,
                                    CLIN_FAR_MOVIMDET      MDE
                                WHERE
                                        MFDE_ID = DEV.MDEV_MFDE_ID
                                  AND MFDE_MEIN_ID = cur.FBOI_MEIN_ID
                                  AND ((dev.MDEV_FECHA) between (TO_DATE(TO_CHAR(v_CKAR_FECHA_APERTURA, 'DD/MON/YY'))) and   v_CKAR_FECHA_CIERRE or v_CKAR_FECHA_APERTURA is null)
                            )
                        ORDER BY
                            MFDE_FECHA
                        /*select MFDE_ID, MDEV_ID, MFDE_FECHA, INT_ERP_ERROR
                                       from (select mde.MFDE_ID , 0 MDEV_ID
                                             , mde.MFDE_MEIN_ID , mde.MFDE_FECHA  MFDE_FECHA, mde.INT_ERP_ERROR INT_ERP_ERROR
                                             from clin_far_movimdet mde
                                             where ((mde.MFDE_FECHA) between (TO_DATE(TO_CHAR(v_CKAR_FECHA_APERTURA, 'DD/MON/YY'))) and   v_CKAR_FECHA_CIERRE or v_CKAR_FECHA_APERTURA is null)
                                             union all
                                             ...
                                             , dev.MDEV_FECHA MFDE_FECHA, dev.INT_ERP_ERROR INT_ERP_ERROR
                                             from clin_far_movim_devol dev
                                             where ((dev.MDEV_FECHA) between (TO_DATE(TO_CHAR(v_CKAR_FECHA_APERTURA, 'DD/MON/YY'))) and   v_CKAR_FECHA_CIERRE or v_CKAR_FECHA_APERTURA is null)
                                             )
                                       where MFDE_MEIN_ID = cur.FBOI_MEIN_ID
                                       order by MFDE_FECHA*/
                        )loop
                            insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('LINEA 172 MFDE_ID : ' || curdet.MFDE_ID || '' , sysdate ); commit;
                            if  v_FBOI_MEIN_ID_aux <> cur.FBOI_MEIN_ID then
                                v_FBOI_MEIN_ID_aux :=  cur.FBOI_MEIN_ID;

                                --SALDO INICIAL
                                begin
                                    INSERT INTO CLIN_FAR_MOV_CONTROLADOS_TMP
                                    ( IDREPORT
                                    ,KADE_CKAR_ID
                                    ,KADE_MEIN_ID
                                    ,KADE_MFDE_FECHA
                            ...
                            if curdet.MDEV_ID > 0 then
                                ...
                                begin
                                    select   MDEV_FECHA, MDEV_CANTIDAD, MDEV_MOVF_TIPO, MDEV_MFDEORIGEN_ID, mdev_referencia_contable
                                    into    v_KADE_MFDE_FECHA, v_KADE_MFDE_CANTIDAD, v_TIPO_MOV, v_MDEV_MFDEORIGEN_ID, v_referencia
                                ...
                                begin
                                    insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('MFDE_ID LINEA 258 : ' || curdet.MFDE_ID || '' , sysdate ); commit;
                                    select nvl(mov.MOVF_CLIID, 0), mov.MOVF_SOLI_ID,  mov.MOVF_BOD_ORIGEN, mov.MOVF_BOD_DESTINO, mov.MOVF_ESTID
                                         ...
                                         ,mde.MFDE_CANTIDAD_DEVUELTA,mde.mfde_referencia_contable,mde.mfde_lote
                                    into   v_KADE_CLIID, v_KADE_MOVF_SOLI_ID, v_MOVF_BOD_ORIGEN, v_MOVF_BOD_DESTINO, v_MOVF_ESTID
                                        ,v_KADE_MOVF_ORCO_NUMDOC,  v_KADE_MOVF_FACTURA_NUMDOC, v_KADE_MOVF_RECETA
                                        ,v_KADE_MFDE_FECHA, v_KADE_MFDE_CANTIDAD, v_TIPO_MOV
                                        ,v_KADE_MFDE_IDTIPOMOTIVO, v_MFDE_SOLI_ID, v_MFDE_MDEV_ID
                                        ,v_MFDE_CANTIDAD_DEVUELTA,v_referencia,v_lote
                                    FROM clin_far_movim    mov
                                       ,clin_far_movimdet mde
                                    WHERE mov.MOVF_ID = mde.MFDE_MOVF_ID
                                      and mde.MFDE_ID = curdet.MFDE_ID;
                                    -- EXCEPTION When NO_DATA_FOUND Then
                                    --     insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('LINEA 279 MFDE_ID : ' || curdet.MFDE_ID || '' , sysdate ); commit;
                                    --     GoTo PKG_Exit;
                                end;
                            ...
                            --81	INGRESOS POR INTERFAZ FIN700
                            if v_TIPO_MOV = 81 then
                                v_KADE_OPERACION:= 'S';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_ORIGEN;
                                v_KADE_MOVF_BOD_SUMINISTRO:=  v_MOVF_BOD_DESTINO;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            -- 201 SALIDA POR RECHAZOS CONSUMO PACIENTE
                            if v_TIPO_MOV = 201 then
                                v_KADE_OPERACION:= 'R';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_DESTINO;
                                v_KADE_MOVF_BOD_SUMINISTRO:= v_MOVF_BOD_DESTINO ;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            -- 85 SALIDA CONSUMO LEGADO (CARGAS MASIVAS SOLICITADA POR FUSAT)
                            if v_TIPO_MOV = 85 then
                                v_KADE_OPERACION:= 'R';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_DESTINO;
                                v_KADE_MOVF_BOD_SUMINISTRO:= v_MOVF_BOD_DESTINO ;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            -- 410 INGRESO POR ANULACION DE DISPENSACION DE RECETAS AMBULATORIA.
                            if v_TIPO_MOV = 410 then
                                v_KADE_OPERACION:= 'S';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_DESTINO;
                                v_KADE_MOVF_BOD_SUMINISTRO:= v_MOVF_BOD_DESTINO ;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            -- 420 INGRESO POR ANULACION DE DISPENSACION DE RECETAS URGENCIA.
                            if v_TIPO_MOV = 420 then
                                v_KADE_OPERACION:= 'S';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_DESTINO;
                                v_KADE_MOVF_BOD_SUMINISTRO:= v_MOVF_BOD_DESTINO ;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            -- 430 INGRESO POR ANULACION DE DISPENSACION DE RECETAS HOSPITALARIA.
                            if v_TIPO_MOV = 430 then
                                v_KADE_OPERACION:= 'S';
                                v_KADE_MOVF_BOD_EXTERNA:= v_MOVF_BOD_DESTINO;
                                v_KADE_MOVF_BOD_SUMINISTRO:= v_MOVF_BOD_DESTINO ;
                                if v_KADE_MOVF_BOD_SUMINISTRO = In_CodBodega then
                            ...
                            v_mfde_id:=curdet.MFDE_ID;
                            v_mdev_id:=curdet.MDEV_ID;
                            ...
                            if v_mfde_id > 0 then
                                Begin
                                    select nvl(LIDE_CORRE_CODMEI,0) into v_correlativo from CLIN_FAR_LIBRO_CONT_DETA where LIDE_MFDE_ID=v_mfde_id;
                                EXCEPTION When NO_DATA_FOUND Then
                                    v_correlativo:=0;
                                end;
                            Else
                                if v_mdev_id > 0 then
                                    Begin
                                        select nvl(LIDE_CORRE_CODMEI,0) into v_correlativo from CLIN_FAR_LIBRO_CONT_DETA where LIDE_MDEV_ID=v_mdev_id;
                                    EXCEPTION When NO_DATA_FOUND Then
                                        v_correlativo:=0;
                                    end;
                                end if;
                            ...
                            if v_KADE_MOVF_RECETA > 0 then
                                Begin
                                    select RECE_TIPDOCPROF, RECE_DOCUMPROF into
                                        v_KADE_CODTIPIDENTIFICACION_PR, v_KADE_CLINUMIDENTIFICACION_PR
                                    from clin_Far_recetas where rece_id=v_KADE_MOVF_RECETA;
                                EXCEPTION When NO_DATA_FOUND Then
                                    v_KADE_CODTIPIDENTIFICACION_PR:=0;
                                    v_KADE_CLINUMIDENTIFICACION_PR:=' ';
                                end;
                                ...
                                begin
                                    select CODTIPIDENTIFICACION, CLINUMIDENTIFICACION
                                    into v_KADE_CODTIPIDENTIFICACION_PR, v_KADE_CLINUMIDENTIFICACION_PR
                                    from cliente where CLIID = v_KADE_PCLIIDMEDSOLICITANTE;
                                Exception
                                    When NO_DATA_FOUND Then
                                        v_KADE_CODTIPIDENTIFICACION_PR:=0;
                                        v_KADE_CLINUMIDENTIFICACION_PR:=' ';
                            ...
                            Begin
                                Select clinombres, cliapepaterno, clIapematerno into
                                    v_KADE_CLINOMBRES_PROF, v_KADE_CLIAPEPATERNO_PROF, v_KADE_CLIAPEMATERNO_PROF
                                from cliente where CODTIPIDENTIFICACION=v_KADE_CODTIPIDENTIFICACION_PR
                                               And CLINUMIDENTIFICACION=rpad(upper(v_KADE_CLINUMIDENTIFICACION_PR),20);
                            EXCEPTION When NO_DATA_FOUND Then
                                Begin
                                    Select clinombres, cliapepaterno, clIapematerno into
                                        v_KADE_CLINOMBRES_PROF, v_KADE_CLIAPEPATERNO_PROF, v_KADE_CLIAPEMATERNO_PROF
                            ...
                            insert into PASO_ERRORES_EO(DESCRIPCION, FECHA ) values ('1) Profesional: '||v_KADE_CODTIPIDENTIFICACION_PR||'-'||v_KADE_CLINUMIDENTIFICACION_PR||', '||v_KADE_CLINOMBRES_PROF||' '||v_KADE_CLIAPEPATERNO_PROF||' '||v_KADE_CLIAPEMATERNO_PROF  , sysdate ); commit;
                            COMMIT;
    ...
    End PROCESA_MOVIMIENTOS_ART_CONTROLADO ;
